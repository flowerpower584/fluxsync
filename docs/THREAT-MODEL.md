...
| STRIDE | Threat | Severity | Status | Ticket / Note |
|---|---|---|---|---|
| **S**poofing            | Same-user local malware reads `identity.bin`, exfiltrates the X25519 secret, and impersonates this device on the LAN against every peer that has paired with it. | **High** | **Open.** `identity.bin` is mode `0600` (`keystore.rs:104`), which only blocks **other unix users** — not the current user's processes, backups, or disk forensics. `SECURITY.md` §1 promises OS keychain storage; not implemented. | **FS-053** |
