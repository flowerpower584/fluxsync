...
// -----------------------------------------------------------------------
// FluxSync patch (vendored copy of tray-icon 0.21.3, upstream otherwise
// unmodified — https://github.com/tauri-apps/tray-icon).
//
// Upstream only opens the tray context menu on a literal rightMouseDown:/
// rightMouseUp:. On macOS, "ctrl + primary click" is the system-wide
// convention for a secondary click, but it arrives as mouseDown:/mouseUp:
// with the Control modifier flag set, so it silently fell through to the
// left-click (popup toggle) path instead of the menu. Confirmed unfixed
// upstream through tray-icon 0.24.1 (latest release at patch time).
//
// Fix: on_mouse_down/on_mouse_up now check NSEvent::modifierFlags() for
// NSEventModifierFlags::Control and, if set, run the same body as
// on_right_mouse_down/on_right_mouse_up instead. See `// FLUXSYNC:` below.
// -----------------------------------------------------------------------
...
use objc2_app_kit::{
    NSCellImagePosition, NSEvent, NSEventModifierFlags, NSImage, NSMenu, NSStatusBar,
...
define_class!(
    ...
    impl TrayTarget {
        ...
        fn on_mouse_down(&self, event: &NSEvent) {
            // FLUXSYNC: ctrl+click == secondary click (macOS convention).
            // Run the exact rightMouseDown: body instead of the left path.
        ...
        fn on_mouse_up(&self, event: &NSEvent) {
            // FLUXSYNC: matching Up half — see on_mouse_down above.
            if event.modifierFlags().contains(NSEventModifierFlags::Control) {
                send_mouse_event(
                    self,
                    event,
