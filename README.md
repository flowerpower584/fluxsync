...
> **Platform status (v0.7.0):** Android, macOS, Windows and Linux all ship a GUI. macOS = menu-bar app, no Dock icon (Tauri v2, built from source — no signed DMG until an Apple Dev ID is funded). Windows = NSIS tray app (x64 + ARM64). Linux = full Tauri GUI (deb / AppImage) plus a native ksni `StatusNotifierItem` system-tray. Android is the first-class mobile client. The daemon + CLI also build cleanly everywhere for headless use.
...
### 📱 Android
1. Download [**FluxSync-v0.7.0-arm64-v8a.apk**](https://github.com/flowerpower584/fluxsync/releases/download/v0.7.0/FluxSync-v0.7.0-arm64-v8a.apk) (~25 MB, arm64-v8a).
...
### 🪟 Windows — tray app
Download the NSIS installer for your arch from the [latest release](https://github.com/flowerpower584/fluxsync/releases/latest): `FluxSync_0.7.0_x64-setup.exe` or `FluxSync_0.7.0_arm64-setup.exe`. It installs per-user and fetches the WebView2 runtime if missing. Unsigned, so SmartScreen shows **More info → Run anyway** on first launch.
