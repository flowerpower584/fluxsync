...
fn test_02_zombie_handshake_after_toggle_off() {
    ...
    assert!(
        actions.is_empty(),
        "Zombie handshake must not trigger actions"
    );
}
...
fn test_04_privacy_leak_cross_peer_history() {
    ...
    assert!(
        ...
        "Privacy Leak: Old history visible to new peer!"
    );
}
...
fn test_16_history_rotation_at_limit() {
    ...
    assert!(
        app.state.history.iter().all(|x| x.preview != "Old 0"),
        "Oldest item was NOT evicted!"
    );
}
...
fn test_36_malformed_wall_clock_time() {
    ...
    assert_eq!(
        app.state.history[0].time,
        "99:99-IMPOSSIBLE-TIME-LONG-STRING"
    );
}
