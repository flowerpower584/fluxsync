...
fn wall() -> StubWallClock { StubWallClock::new("14:32", 1_700_000_000_000) }
fn boot() -> App { App::new(Config::default()) }
fn h(seed: u8) -> [u8; 32] { [seed; 32] }

fn linked_app() -> App {
    let mut app = boot();
    app.handle(Event::ToggleOn, &wall());
    app.handle(Event::PeerSeen { peer_id: [7; 32], name: "Galaxy".into() }, &wall());
    app.handle(Event::HandshakeOk, &wall());
    app.handle(Event::BatteryChangedSelf { level: 80, charging: false }, &wall());
    app.handle(Event::BatteryChangedPeer { level: 80, charging: false }, &wall());
    app
}
...
fn lifecycle_full_happy_path() {
    ...
    let a = app.handle(Event::PeerSeen { peer_id: [1;32], name: "SM-G998N".into() }, &wall());
...
fn lifecycle_handshake_timeout_retries() {
    let mut app = boot();
    app.handle(Event::ToggleOn, &wall());
    app.handle(Event::PeerSeen { peer_id: [1;32], name: "P".into() }, &wall());
    assert_eq!(app.phase, Phase::Handshaking);

    let a = app.handle(Event::HandshakeTimeout, &wall());
    assert_eq!(app.phase, Phase::Discovering);
    assert!(a.iter().any(|x| matches!(x, Action::EmitLog(e) if e.level == LogLevel::Warn)));
}
...
fn toggle_off_from_every_phase() {
    for start_phase in [Phase::Idle, Phase::Discovering, Phase::Handshaking, Phase::Linked, Phase::Paused, Phase::Halted] {
...
fn make_state(on: bool, sb: u8, sc: bool, pb: u8, pc: bool, thr: u8) -> State {
    ...
    s.on = on; s.battery_level = sb; s.charging = sc;
    s.peer_battery = pb; s.peer_charging = pc; s.battery_threshold = thr;
    s
}
...
fn policy_inactive_always_when_off() {
    for b in [0, 5, 15, 50, 100] {
        assert_eq!(status_for(&make_state(false, b, false, b, false, 20)), Status::Inactive);
    }
}
...
fn policy_critical_at_5_or_below_either_side() {
    for lvl in 0..=5u8 {
        assert_eq!(status_for(&make_state(true, lvl, false, 80, false, 20)), Status::Critical);
        assert_eq!(status_for(&make_state(true, 80, false, lvl, false, 20)), Status::Critical);
        // Critical even when charging
        assert_eq!(status_for(&make_state(true, lvl, true, 80, false, 20)), Status::Critical);
    }
}
...
fn policy_paused_at_threshold_boundary() {
    let thr = 20u8;
    // At threshold = Paused
    assert_eq!(status_for(&make_state(true, thr, false, 80, false, thr)), Status::Paused);
    // One above = Syncing
    assert_eq!(status_for(&make_state(true, thr+1, false, 80, false, thr)), Status::Syncing);
    // Same for peer side
    assert_eq!(status_for(&make_state(true, 80, false, thr, false, thr)), Status::Paused);
    assert_eq!(status_for(&make_state(true, 80, false, thr+1, false, thr)), Status::Syncing);
}

#[test]
fn policy_charging_overrides_pause() {
    assert_eq!(status_for(&make_state(true, 10, true, 80, false, 20)), Status::Syncing);
    assert_eq!(status_for(&make_state(true, 80, false, 10, true, 20)), Status::Syncing);
}

#[test]
fn policy_both_below_still_paused() {
    assert_eq!(status_for(&make_state(true, 15, false, 15, false, 20)), Status::Paused);
}
...
fn policy_phase_mirrors_status_after_link() {
    ...
    app.handle(Event::BatteryChangedSelf { level: 10, charging: false }, &wall());
    assert_eq!(app.snapshot().status, Status::Paused);
    assert_eq!(app.phase, Phase::Paused);

    // Charge → back to linked
    app.handle(Event::BatteryChangedSelf { level: 10, charging: true }, &wall());
    assert_eq!(app.snapshot().status, Status::Syncing);
    assert_eq!(app.phase, Phase::Linked);

    // Drop to critical
    app.handle(Event::BatteryChangedSelf { level: 3, charging: false }, &wall());
...
fn clipboard_local_to_peer_emits_send() {
    ...
    let a = app.handle(Event::LocalClipboardChange {
        hash: h(1), kind: Kind::Text, preview: "Salam".into(), sensitive: false, lamport: 1
    }, &wall());
    assert!(a.iter().any(|x| matches!(x, Action::SendItem { hash, .. } if *hash == h(1))));
...
fn clipboard_dedup_suppresses_echo() {
    let mut app = linked_app();
    let a1 = app.handle(Event::LocalClipboardChange {
        hash: h(10), kind: Kind::Text, preview: "x".into(), sensitive: false, lamport: 1
    }, &wall());
    assert!(a1.iter().any(|x| matches!(x, Action::SendItem { .. })));

    // Same hash = suppressed
    let a2 = app.handle(Event::LocalClipboardChange {
        hash: h(10), kind: Kind::Text, preview: "x".into(), sensitive: false, lamport: 2
    }, &wall());
...
fn clipboard_cross_dedup_peer_then_local() {
    ...
    let a = app.handle(Event::LocalClipboardChange {
        hash: h(20), kind: Kind::Text, preview: "echo".into(), sensitive: false, lamport: 2
    }, &wall());
...
fn clipboard_sensitive_not_in_history() {
    ...
    app.handle(Event::LocalClipboardChange {
        hash: h(30), kind: Kind::Text,
        preview: "sk_live_aBcDeFgHiJkLmNoPqRsTuVwX".into(),
        sensitive: true, lamport: 1
    }, &wall());
...
fn state_json_has_all_required_keys() {
    ...
    for key in ["on", "battery_level", "battery_threshold", "charging",
                "peer_name", "peer_battery", "peer_charging", "history",
                "status", "version", "link_latency_ms", "cipher"] {
...
fn fsm_noop_for_invalid_transitions() {
    ...
    let (p, a) = transition(Phase::Idle, &Event::PeerSeen { peer_id: [1;32], name: "X".into() });
    assert_eq!(p, Phase::Idle);
    assert!(a.is_empty());
...
fn fsm_peer_seen_in_linked_emits_state() {
    let (p, a) = transition(Phase::Linked, &Event::PeerSeen { peer_id: [1;32], name: "X".into() });
    assert_eq!(p, Phase::Linked);
...
fn emit_state_on_every_meaningful_event() {
    ...
    let a = app.handle(Event::BatteryChangedSelf { level: 50, charging: false }, &wall());
...
fn stress_interleaved_clipboard_and_battery() {
    ...
    for i in 0..200u16 {
        ...
        if i % 3 == 0 {
            app.handle(Event::BatteryChangedSelf {
                level: ((i % 95) + 6) as u8, charging: i % 7 == 0
            }, &wall());
        } else if i % 3 == 1 {
            app.handle(Event::LocalClipboardChange {
                hash, kind: Kind::Text, preview: format!("local-{i}"),
                sensitive: false, lamport: u64::from(i)
            }, &wall());
        } else {
            app.handle(Event::FrameReceivedClipboard {
...
fn stress_peer_bounce_10_cycles() {
    ...
    for i in 0..10u8 {
        app.handle(Event::PeerSeen { peer_id: [i; 32], name: format!("Peer-{i}") }, &wall());
        app.handle(Event::HandshakeOk, &wall());
        app.handle(Event::BatteryChangedSelf { level: 80, charging: false }, &wall());
        assert_eq!(app.phase, Phase::Linked);
