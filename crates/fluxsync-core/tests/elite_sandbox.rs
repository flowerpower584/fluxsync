#[cfg(test)]
mod elite_sandbox {
    use fluxsync_core::{App, Config, Event, Action, Clock, Kind};
    use fluxsync_core::clock::WallClock;

    struct StubWall(String);
    impl WallClock for StubWall {
        fn hhmm(&self) -> String { self.0.clone() }
        fn unix_millis(&self) -> u64 { 0 }
    }
    ...
    fn test_lamport_replay_guard() {
        ...
        let actions = app.handle(Event::FrameReceivedClipboard {
            hash: [2; 32],
            kind: Kind::Text,
        ...
        let write_emitted = actions.iter().any(|a| matches!(a, Action::WriteClipboard { .. }));
        assert!(!write_emitted, "Stale Lamport event should be rejected");
        
        // Verify we got a warning log instead.
        let log_emitted = actions.iter().any(|a| matches!(a, Action::EmitLog(l) if l.msg.contains("Rejected stale item")));
        assert!(log_emitted, "Should emit a warning log for rejected stale items");
    }
    ...
    fn test_fsm_guard_on_duplicates() {
        ...
        app.handle(Event::PeerSeen { peer_id: [7; 32], name: "Peer".into() }, &wall);
        ...
        let actions2 = app.handle(Event::FrameReceivedClipboard {
            hash,
            kind: Kind::Text,
            preview: "First".into(),
            lamport: 11,
        ...
        assert!(!actions2.iter().any(|a| matches!(a, Action::WriteClipboard { .. })));
