...
fn the_amnesia_test() {
    ...
    // See peer
    app.handle(Event::PeerSeen { peer_id: [1; 32], name: "Target".into() }, &wall);
    ...
    app.handle(Event::UntrustedPeerSeen { name: "Target".into() }, &wall);
    
    // FSM should have dropped back to Discovering because of UntrustedPeerSeen.
    assert_eq!(app.state.peer_name, "", "App pre-transition logic should have cleared peer_name");
    assert_eq!(app.phase, Phase::Discovering, "FSM should have dropped back to Discovering");
    
    // If we are stuck in Handshaking with no peer_name, we should at least timeout.
    app.handle(Event::HandshakeTimeout, &wall);
    assert_eq!(app.phase, Phase::Discovering, "HandshakeTimeout should rescue us");
}
...
fn the_time_paradox_test() {
    ...
    assert!(app.clock.now() > 101, "Clock should continue to tick forward: {}", app.clock.now());
}
...
fn the_battery_spam_test() {
    ...
    for i in 0..100 {
        app.handle(Event::BatteryChangedSelf { level: i as u8, charging: i % 2 == 0 }, &wall);
    }
...
fn the_zombie_handshake_test() {
    ...
    assert_eq!(app.phase, Phase::Idle);
    assert!(actions.is_empty(), "Should not perform any actions for unexpected handshakes");
}
...
fn the_identity_crisis_test() {
    ...
    // See Peer A
    app.handle(Event::PeerSeen { peer_id: [0xA; 32], name: "Peer A".into() }, &wall);
    assert_eq!(app.state.peer_name, "Peer A");
    
    // SUDDENLY! Before handshake completes, see Peer B.
    app.handle(Event::PeerSeen { peer_id: [0xB; 32], name: "Peer B".into() }, &wall);
    
    // The FSM prevents identity mismatch!
    assert_eq!(app.phase, Phase::Handshaking);
    
    // peer_name should STILL be Peer A because we rejected Peer B due to mismatch.
    assert_eq!(app.state.peer_name, "Peer A", "Identity mismatch prevention failed!");
}
...
fn the_duplicate_history_bug_test() {
    ...
    assert_eq!(app.state.history.len(), 1, "History should NOT contain duplicate entries for the same hash!");
}
...
fn the_privacy_leak_test() {
    ...
    app.handle(Event::ToggleOn, &wall);
    app.handle(Event::PeerSeen { peer_id: [0xA; 32], name: "Private Mac".into() }, &wall);
    app.handle(Event::HandshakeOk, &wall);
    ...
    app.handle(Event::ToggleOn, &wall);
    app.handle(Event::PeerSeen { peer_id: [0xB; 32], name: "Friend Phone".into() }, &wall);
    
    // BUG DETECTION: Is the password still in history?
    // If yes, and the new peer triggers a "BurstReplay", the secret is LEAKED.
    assert!(app.state.history.is_empty(), "HISTORY SHOULD BE CLEARED ON UNPAIR/NEW PAIR! Found: {:?}", app.state.history);
}
...
fn memory_and_cpu_stress_test() {
    ...
    for i in 0..100 {
        app.handle(Event::BatteryChangedSelf { level: i as u8, charging: true }, &wall);
    }
    ...
    assert!(duration.as_millis() < 500, "Performance too slow! State cloning is killing the CPU: {:?}", duration);
}
