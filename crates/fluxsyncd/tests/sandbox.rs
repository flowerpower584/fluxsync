...
async fn test_aggressive_probing_architecture() -> Result<()> {
    ...
    println!(
        "⏱️ Reconnexion réussie en {}ms (contre 30s de timeout auparavant)",
...
async fn test_neural_discovery_fabric() -> Result<()> {
    ...
    println!(
        "🏠 Mode 'Home WiFi' détecté. Intervalle de probing réduit à {}s",
        interval.as_secs()
    );
...
async fn test_sadistic_packet_storm() -> Result<()> {
    ...
    let start = Instant::now();
    for i in 0..5000 {
        // On monte à 5000 pour être vraiment méchant
        attacker
    ...
    println!(
        "✅ Orage REEL terminé en {}ms. Zéro crash, 200% confiance.",
        start.elapsed().as_millis()
    );
    Ok(())
}
